...
class RelatorioPro:
    ...
    def mensal(
        self,
        dados_mes: list[ModelRelatorio],
        dados_mes_anterior: list[ModelRelatorio],
        total_periodo: dict[str, dict[str, int]],
    ):
        """
        ...
        self.planilha[f"A{linha_atual}"].value = (
            f"Previsão de Faturamento {mes}/{ano}".upper()
        )
        self.planilha[f"A{linha_atual}"].alignment = Alignment(horizontal="center")
        self.planilha[f"A{linha_atual}"].font = Font(bold=True)
        self.planilha[f"A{linha_atual}"].fill = PatternFill(
        ...
        self.utils.style_font_planilha(
            self.planilha, linha_atual, linha_atual, coluna_inicio, coluna_fim
        )
        ...
        linha_final = self.planilha.max_row
        self.utils.style_real_planilha(
            self.planilha, linha_atual + 1, linha_final, 2, 2, self.real_style
        )
        self.utils.style_alinhar_planilha(
            self.planilha, linha_atual + 1, linha_final, 3, 5
        )
        self.utils.style_font_planilha(
            self.planilha, linha_final - 2, linha_final, coluna_inicio, 2
        )
        self.utils.style_borda_planilha(
            self.planilha, 1, linha_final, coluna_inicio, coluna_fim
        )
        self.utils.ajustar_colunas_planilha(
            self.planilha, linha_atual, linha_final, coluna_inicio, coluna_fim
        )
        ...
        if comparar_ano_anterior:
            self.planilha[f"A{linha_atual}"].value = (
                f"Faturamento Meses Anteriores {mes_anterior}/{ano - 1}".upper()
            )
        else:
            self.planilha[f"A{linha_atual}"].value = (
                f"Faturamento Meses Anteriores {mes_anterior}/{ano}".upper()
            )
        ...
        self.utils.style_font_planilha(
            self.planilha, linha_final, linha_final, coluna_inicio, 2
        )
        self.utils.style_borda_planilha(
            self.planilha, linha_atual - 1, linha_final, coluna_inicio, coluna_fim
        )
        ...
        self.utils.style_font_planilha(
            self.planilha, linha_atual + 1, linha_final, coluna_inicio, 2
        )
        self.utils.style_borda_planilha(
            self.planilha, linha_atual + 1, linha_final, coluna_inicio, coluna_fim
        )
    ...
    def mensal_separado(
        ...
        for tipo, dados in dicionario.items():
            ...
            self.planilha_separada[f"A{linha_atual}"].value = (
                f"Previsão de Faturamento {tipo} {mes}/{ano}".upper()
            )
            self.planilha_separada[f"A{linha_atual}"].alignment = Alignment(
                horizontal="center"
            )
            self.planilha_separada[f"A{linha_atual}"].font = Font(bold=True)
            self.planilha_separada[f"A{linha_atual}"].fill = PatternFill(
                start_color="FFFF00", end_color="FFFF00", fill_type="solid"
            )
            ...
            self.planilha_separada.append(
                ["TOTAL", f"=SUM(B{linha_atual+1}:B{linha_final})"]
            )
            ...
            self.utils.style_alinhar_planilha(
                self.planilha_separada, linha_atual + 1, linha_final, 3, 5
            )
            self.utils.style_font_planilha(
                self.planilha_separada, linha_final - 2, linha_final, coluna_inicio, 2
            )
            self.utils.style_borda_planilha(
                self.planilha_separada,
                linha_atual - 1,
        ...
        self.utils.style_real_planilha(
            self.planilha_separada, linha_atual + 1, linha_final, 2, 2, self.real_style
        )
        self.utils.style_font_planilha(
            self.planilha_separada, linha_final - 2, linha_final, coluna_inicio, 2
        )
        ...
        for tipo, dados in dicionario.items():
            ...
            if comparar_ano_anterior:
                self.planilha_separada[f"A{linha_atual}"].value = (
                    f"Faturamento Meses Anteriores {tipo} {mes_anterior}/{ano - 1}".upper()
                )
            else:
                self.planilha_separada[f"A{linha_atual}"].value = (
                    f"Faturamento Meses Anteriores {tipo} {mes_anterior}/{ano}".upper()
                )
            ...
            self.utils.style_font_planilha(
                self.planilha_separada, linha_final, linha_final, coluna_inicio, 2
            )
            self.utils.style_borda_planilha(
                self.planilha_separada,
                linha_atual - 1,
            ...
            self.utils.style_font_planilha(
                self.planilha_separada, linha_atual + 1, linha_final, coluna_inicio, 2
            )
            self.utils.style_borda_planilha(
                self.planilha_separada,
                linha_atual + 1,
