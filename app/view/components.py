...
class ViewComponents:
    ...
    def criar_botao(
        ...
        if disable:
            botao = ctk.CTkButton(
                janela, text=text, command=func, state="disabled", fg_color="gray"
            )
    ...
    def criar_entrys(
        self, lista: list | tuple, coluna: int, linha: int, janela: ctk.CTk
    ) -> None:
        """
    ...
    def criar_entry_opcao(
        self,
        janela: ctk.CTk,
        coluna: int,
        linha: int,
        quantidade_opcao: int | list,
        texto: str,
    ) -> None:
        """
    ...
    def criar_entry_data(
        self, janela: ctk.CTk, coluna: int, linha: int, text: str
    ) -> None:
        """
        ...
        def popup_calendario(entry: ctk.CTkEntry) -> None:
            ...
            def selecionar_data() -> None:
                ...
                if text == "Data de Faturamento":
                    ...
                    self.entrys["Mês de Referência"].set(
                        self.utils.MESES[data.month - 1]
                    )
        ...
        botao = ctk.CTkButton(
            janela,
            text=f"{calendario_emoji}",
            command=lambda: popup_calendario(camp_data),
            font=("Arial", 20),
        )
