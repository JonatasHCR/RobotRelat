...
from app.view.view_cliente import ViewCliente
from app.view.view_nota import ViewNota
...
class App(ctk.CTk):
    ...
    def __init__(self, controller: ControllerRelatorio) -> None:
        ...
        self.components.criar_botao(
            "Cliente", lambda: self.client.janela(self), 0, 0, self
        )
        self.components.criar_botao("Nota", lambda: self.nota.janela(self), 0, 1, self)
        self.components.criar_botao(
            "Relatório", lambda: self.janela_relatorio(self, 0), 0, 3, self
        )
    ...
    def janela_relatorio(self, janela_main: ctk.CTk, pagina: int) -> None:
        ...
        self.components.criar_botao(
            "Gerar Relatório",
            lambda: self.controller.relatorio(entry_mes, entry_ano),
            2,
            0,
            janela_secundaria,
        )
        ...
        for coluna in self.utils.colunas_all:
            col = ctk.CTkLabel(
                janela_secundaria, text=coluna, font=("Arial", 16, "bold")
            )
        ...
        for n in range(self.controller.contar_pagina()):
            ...
            if n == pagina:
                self.components.criar_botao(
                    f"{n+1}",
                    lambda pagina=n: self.retirar(janela_secundaria, pagina),
                    coluna,
                    linha,
                    janela_secundaria,
                    True,
    ...
    def retirar(self, janela: ctk.CTk, pagina: int) -> None:
        ...
        for n in range(self.controller.contar_pagina()):
            ...
            if n == pagina:
                self.components.criar_botao(
                    f"{n+1}",
                    lambda pagina=n: self.retirar(janela, pagina),
                    coluna,
                    linha,
                    janela,
                    True,
