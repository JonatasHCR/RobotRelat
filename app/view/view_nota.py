...
from app.controller.controller_nota import ControllerNota
from app.view.components import ViewComponents
...
class ViewNota:
    ...
    def janela(self, janela_main: ctk.CTk) -> None:
        ...
        self.components.criar_botao(
            "Cadastrar",
            lambda: self.janela_form(janela_secundaria),
            0,
            0,
            janela_secundaria,
        )
        self.components.criar_botao(
            "Alterar",
            lambda: self.janela_alterar(janela_secundaria, 0),
            1,
            0,
            janela_secundaria,
        )
        self.components.criar_botao(
            "Visualizar",
            lambda: self.janela_visualizar(janela_secundaria, 0),
            2,
            0,
            janela_secundaria,
        )
    ...
    def janela_form(self, janela: ctk.CTkToplevel) -> None:
        ...
        self.components.criar_entry_opcao(
            janela, coluna, linha, 20, "Quantidade de Registros"
        )
        self.components.criar_entrys(
            ["Centro de Custo", "Numero da Nota", "Valor da Nota"],
            coluna,
            linha + 1,
            janela,
        )
        self.components.criar_entry_data(
            janela, coluna, linha + 4, "Data de Faturamento"
        )
        self.components.criar_entry_data(janela, coluna, linha + 5, "Data de Pagamento")
        self.components.criar_entry_opcao(
            janela, coluna, linha + 6, self.utils.MESES, "Mês de Referência"
        )
        self.components.criar_entry(
            "Ano de Referência", 0, linha + 7, janela, self.utils.pegar_ano_atual()
        )

        # botão de envio
        self.components.criar_botao(
            "Enviar",
            lambda: self.controller.cadastrar(texto_feedback, self.components.entrys),
            coluna + 1,
            linha + 8,
            janela,
        )
    ...
    def janela_alterar(self, janela: ctk.CTkToplevel, pagina: int):
        ...
        for nota in notas:
            ...
            self.components.criar_botao(
                "Deletar",
                lambda nota=nota: self.popup_deletar(janela, nota),
                coluna - 1,
                linha,
                janela,
            )
        ...
        for n in range(self.controller.contar_pagina()):
            ...
            if n == pagina:
                self.components.criar_botao(
                    f"{n+1}",
                    lambda pagina=n: self.janela_alterar(janela, pagina),
                    coluna,
                    linha,
                    janela,
                    True,
        ...
        def popup_confirmacao() -> None:
            ...
            texto = ctk.CTkLabel(
                janela_popup, text="Tem certeza que deseja alterar os dados das notas ?"
            )
        ...
        self.components.criar_botao(
            "Alterar Dados", popup_confirmacao, 0, linha + 1, janela
        )
    ...
    def janela_visualizar(self, janela: ctk.CTkToplevel, pagina: int) -> None:
        ...
        for n in range(self.controller.contar_pagina()):
            ...
            if n == pagina:
                self.components.criar_botao(
                    f"{n+1}",
                    lambda pagina=n: self.janela_visualizar(janela, pagina),
                    coluna,
                    linha,
                    janela,
                    True,
    ...
    def popup_deletar(self, janela: ctk.CTkToplevel, nota: ModelNota) -> None:
        def popup_confirmacao() -> None:
            ...
            texto = ctk.CTkLabel(
                janela_popup,
                text="Tem certeza que deseja DELETAR a nota ?",
                fg_color="red",
            )
            texto.grid(column=0, row=0, pady=10, padx=10)

            self.components.criar_botao(
                "Não Deletar", janela_popup.destroy, 0, 1, janela_popup
            )

            botao = ctk.CTkButton(
                janela_popup, text="DELETAR", command=aux_func, fg_color="red"
            )
            botao.grid(column=1, row=1, pady=10, padx=10)
